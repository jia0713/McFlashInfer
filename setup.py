...
def get_version():
    ...
    # Get PyTorch version (major.minor only)
    torch_version = ''.join(torch.__version__.split('.')[:3])[:3]

    return f"{package_version}+{git_sha}.torch{torch_version}"
...
if enable_aot:
    ...
    def get_cuda_version() -> Version:
        if torch_cpp_ext.CUDA_HOME is None:
            ...
            nvcc = os.path.join(torch_cpp_ext.CUDA_HOME, "bin/cucc")
        return Version("1.0")
