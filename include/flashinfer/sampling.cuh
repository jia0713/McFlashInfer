...
#else
    // Because version mismatch using 'SubtractLeft' will be counted incorrectly;
    // 'FlagHedads' is about to be deprecated, it will throw a warning at compile time,
...
#pragma unroll
      ...
      aggregate_gt_pivot +=
          BlockReduce<DType, BLOCK_THREADS>(temp_storage.block_prim.reduce).Sum(probs_gt_pivot);
      if (tx == 0) {
