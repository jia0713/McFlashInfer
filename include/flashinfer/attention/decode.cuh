...
#pragma unroll
  for (uint32_t j = 0; j < tile_size; ++j) {
    ...
    float vec_s[2] = {0.f, 0.f};
#pragma unroll
    for (uint32_t i = 0; i < vec_size; i += 2) {
      fma_f32x2(vec_s, &q_vec[i], &k_vec[i], vec_s);
    }
    s[j] = vec_s[0] + vec_s[1];
#pragma unroll
...
#pragma unroll
    for (uint32_t i = 0; i < vec_size; i += 2) {
      fma_f32x2(&st.o[i], &st.o[i], o_scale);
    }
  }
}
...
#pragma unroll
    for (uint32_t j = 0; j < tile_size_per_bdx; ++j) {
      kv_offset[j] =
          kv_offset_smem[iter * bdz * bdy * tile_size_per_bdx + tz * bdy + ty + j * bdy * bdz] +
          tx * vec_size;
    }
#pragma unroll
    for (uint32_t j = 0; j < tile_size_per_bdx; ++j) {
      cp_async::load_128b_bsm_pred(
          k_smem +
              (stage_idx * bdz * bdy * tile_size_per_bdx + tz * bdy + ty + j * bdy * bdz) *
                  head_dim +
              tx * vec_size,
          paged_kv.k_data + kv_offset[j],
          iter * bdz * bdy * tile_size_per_bdx + tz * bdy + ty + j * bdy * bdz < chunk_size);
    }
#pragma unroll
    for (uint32_t j = 0; j < tile_size_per_bdx; ++j) {
      cp_async::load_128b_bsm_pred(
...
#pragma unroll
    ...
    // compute qk
    cp_async_bsm_wait<num_stages_smem / 2 * tile_size_per_bdx * 2>();
    compute_qk<POS_ENCODING_MODE, vec_size, bdx, bdy * tile_size_per_bdx>(
...
#pragma unroll
    for (uint32_t j = 0; j < tile_size_per_bdx; ++j) {
      kv_offset[j] =
          kv_offset_smem[((iter + num_stages_smem) % bdx) * bdz * bdy * tile_size_per_bdx +
                         tz * bdy + ty + j * bdy * bdz] +
          tx * vec_size;
    }
    ...
    for (uint32_t j = 0; j < tile_size_per_bdx; ++j) {
      cp_async::load_128b_bsm_pred(
          k_smem +
              ...
              tx * vec_size,
          paged_kv.k_data + kv_offset[j],
          (iter + num_stages_smem) * bdz * bdy * tile_size_per_bdx + tz * bdy + ty + j * bdy * bdz <
              chunk_size);
    }
