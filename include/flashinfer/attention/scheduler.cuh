...
inline cudaError_t BatchDecodeWithPagedKVCacheWorkEstimationDispatched(
  ...
  auto use_double_buff = false;
  DISPATCH_DECODE_NUM_STAGES_SMEM(use_double_buff, NUM_STAGES_SMEM, {
...
inline auto PrefillSplitQOKVIndptr(IdType* qo_indptr_h, IdType* kv_indptr_h,
                                   ...
                                   uint32_t head_dim_qk, uint32_t head_dim, uint32_t page_size,
  ...
  if (enable_cuda_graph) {
    ...
    cta_tile_q = FA2DetermineCtaTileQ(max_qo_len, head_dim_qk > head_dim);
    ...
    cta_tile_q = FA2DetermineCtaTileQ(avg_packed_qo_len, head_dim_qk > head_dim);
...
inline cudaError_t PrefillPlan(void* float_buffer, size_t float_workspace_size_in_bytes,
  ...
  auto [split_kv, new_batch_size, padded_batch_size, cta_tile_q, kv_chunk_size, request_indices_vec,
      ...
      PrefillSplitQOKVIndptr(qo_indptr_h, kv_indptr_h, total_num_rows, batch_size, num_qo_heads,
                             num_kv_heads, head_dim_qk, head_dim_vo, page_size,
...
inline cudaError_t MLAPlan(void* float_buffer, size_t float_workspace_size_in_bytes,
  ...
  int cluster_size;
  int smem_size = GetSharedMemorySize();
  const int cta_tile_q = [&]() {
    if (smem_size == 128 * 1024) {
      return 64;
    } else if (smem_size == 64 * 1024) {
      if (batch_size < 16 && avg_packed_qo_len <= 128) {
        return 32;
      } else {
        return 64;
      }
      ...
      err << "Unsupported shared memory size: " << smem_size;
  ...
  if (avg_packed_qo_len > 64) {
    cluster_size = 2 * 64 / cta_tile_q;  // two ctas in a cluster
  } else {
    cluster_size = 1 * 64 / cta_tile_q;  // one cta in a cluster
  }
  ...
  std::vector<ClusterInfo> short_chunks;
  short_chunks.reserve(batch_size);
    ...
    for (int qo_tile_idx = num_qo_tiles - 1; qo_tile_idx >= 0; --qo_tile_idx) {
      ...
      while (remaining_len > 0 || zero_kv_len) {
        ...
        if (remaining_len >= kv_len_limit) {
          ...
          cluster_partial_indptr[cluster_idx].push_back(split_kv ? partial_o_nnz : -1);
          ...
          cluster_cost_heap.insert({cluster_idx, accum_cost});
          short_chunks.emplace_back(ClusterInfo{actual_len, qo_len, kv_len, qo_indptr_h[i],
                                                kv_indptr_h[i], (split_kv ? partial_o_nnz : -1),
                                                qo_tile_idx * cluster_tile_q, kv_start,
                                                kv_start + actual_len, split_kv});
        }
        if (split_kv) {
  ...
  for (const auto& op : short_chunks) {
    auto [cluster_idx, accum_cost] = cluster_cost_heap.pop();
    cluster_cost_heap.insert(
        {cluster_idx, accum_cost + cost_function(cluster_tile_q, op.actual_len)});
    cluster_q_len[cluster_idx].push_back(op.qo_len);
    cluster_kv_len[cluster_idx].push_back(op.kv_len);
    cluster_q_indptr[cluster_idx].push_back(op.qo_indptr_val);
    cluster_kv_indptr[cluster_idx].push_back(op.kv_indptr_val);
    cluster_partial_indptr[cluster_idx].push_back(op.partial_o_nnz_val);
    cluster_q_start[cluster_idx].push_back(op.qo_tile_start);
    cluster_kv_start[cluster_idx].push_back(op.kv_start_val);
    cluster_kv_end[cluster_idx].push_back(op.kv_end_val);
  }
  short_chunks.clear();
  ...
  int max_total_num_works = 32768;  // NOTE(Zihao): adjust it later
