...
#define DISPATCH_WEIGHT_LAYOUT(is_column_major, WEIGHT_LAYOUT, ...) \
  if (is_column_major) {                                            \
    using WEIGHT_LAYOUT = mctlass::layout::ColumnMajor;             \
    __VA_ARGS__                                                     \
  } else {                                                          \
    using WEIGHT_LAYOUT = mctlass::layout::RowMajor;                \
    __VA_ARGS__                                                     \
  }
...
cudaError_t CutlassSegmentGEMMRun(void* workspace_buffer, size_t workspace_buffer_size_in_bytes,
                                  ...
                                  cudaStream_t stream) {
  using mctlass::epilogue::thread::LinearCombination;
  using mctlass::gemm::threadblock::GemmIdentityThreadblockSwizzle;
  int device;
  ...
  DISPATCH_WEIGHT_LAYOUT(weight_column_major, WEIGHT_LAYOUT, {
    DISPATCH_SMEM_CONFIG(smem_limit_per_sm, NUM_STAGES, {
      using GemmKernel = typename mctlass::gemm::kernel::DefaultGemmGrouped<
          DType,                                   // Element A
          mctlass::layout::RowMajor,               // Layout A
          ...
          8,                                       // Granularity B
          DType,                                   // Element C&D
          mctlass::layout::RowMajor,               // Layout C&D
          float,                                   // Element Accumulator
          mctlass::arch::OpClassTensorOp,          // Operator Class Tag
          mctlass::arch::Sm80,                     // Architecture
          mctlass::gemm::GemmShape<128, 128, 32>,  // Thread Block Shape
          mctlass::gemm::GemmShape<64, 64, 32>,    // Warp Shape
          mctlass::gemm::GemmShape<16, 8, 16>,     // Instruction Shape
          mctlass::epilogue::thread::LinearCombination<DType, 8, float, float>,  // Epilogue
          mctlass::gemm::threadblock::GemmBatchedIdentityThreadblockSwizzle,  // Swizzling Operator
          NUM_STAGES                                                          // Stages
          >::GemmKernel;

      using EpilogueOutputOp = typename GemmKernel::Epilogue::OutputOp;
      typename EpilogueOutputOp::Params epilogue_op(1.0, 1.0);
      using GemmGrouped = mctlass::gemm::device::GemmGrouped<GemmKernel>;
      typename GemmGrouped::Arguments args(
          reinterpret_cast<mctlass::gemm::GemmCoord*>(all_problems), (int)batch_size,
      ...
      if (status != mctlass::Status::kSuccess) {
        ...
        err_msg << "mctlass group_gemm.initialize failed: " << mctlassGetStatusString(status);
        FLASHINFER_ERROR(err_msg.str());
      }
      status = gemm.run(stream);
      if (status != mctlass::Status::kSuccess) {
        std::ostringstream err_msg;
        err_msg << "mctlass group_gemm.run failed: " << mctlassGetStatusString(status);
        FLASHINFER_ERROR(err_msg.str());
      }
    });
  });
