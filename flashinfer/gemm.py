    ...
    def run(
        ...
        if backend == "sm90":
            ...
            for i in range(batch_size):
                x_data = x[seg_indptr[i] : seg_indptr[i + 1]]
                if weight_indices.numel() == 0:
                    out[seg_indptr[i] : seg_indptr[i + 1]] = torch.mm(
                        x_data, weights[i].T if weight_column_major else weights[i]
                    )
                else:
                    out[seg_indptr[i] : seg_indptr[i + 1]] = torch.mm(
                        x_data,
                        (
                            weights[weight_indices[i]].T
                            if weight_column_major
                            else weights[weight_indices[i]]
                        ),
                    )
        else:
